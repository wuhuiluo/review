...
// 分为三种类型 反射型xss代码在请求URL中攻击 存储型： 将攻击脚本存入服务器， 进行传播 文档型： 通过DOM修改页面内容
// XSS跨站脚本攻击
...
// 4.使用验证码：防止脚本冒充用户做一些危险操作
// CSRF跨站请求伪造
// 用户登陆一个网站后，产生一个cookie，此时又打开了新的网站，这个新的网站产生了一些恶意的请求
// 用户登陆一个网站之后，产生一个cookie，这时候打开了一个新的网址，该网址返回了一些恶意的请求就属于CSRF攻击,cookie并不能获取到只是使用cookie
// 用户登陆一个网站之后，残生了一个cookie，这时候打开了一个新网站，该网站返回了一些恶意的请求这就属于CSRF攻击

// 防范措施
// 利用cookie的samesite属性
// 验证来源站点通过origin header referer header
// CSRF TOKEN页面提交请求的时候验证token合不合法
// 利用Cookie设置SameSite属性
// strict: 禁止第三方携带cookie
// lax: get方法和a标签可以携带cookie
// none: 默认会自动携带上cookie
...
// 如果JS运行在源头A里面， 就只能获取资源A的数据， 不能获取资源B数据， 不允许跨域

// 同源的根源：无法区分谁是发送者 严格限制同源策略

// CORS跨域资源共享 客服AJAX只能同源使用的限制

// CORS是跨域资源共享，他允许浏览器像跨院服务器发送请求，客服了AJAX只能同源使用的限制
// 如何理解CORS

// 101: 在HTTP升级为websocket的时候，服务器统一变更返回10 1

// 200：客户端的请求在服务器被正确处理
// 204：客户端请求成功，但是没有资源返回
// 206：客户端进行范围请求，服务器执行了这个部分的GET 请求

// 301 永久重定向，资源被分配给了新的URL
// 302：临时重定向，资源被临时分配给了新的URL
// 303：资源存在另一个URL，需要用get方法来获取
// 304：服务器告诉客户端我的资源没有改变，你直接用你本地的就跨域
// 307： 临时重定向，资源被临时分配给了的新的URL

// 400：请求报文存在语法错误
// 401：客户端的请求没有HTTP认证的认证信息
// 403：客户端的资源请求被服务器剞劂
// 404：在服务器上没有找到该资源
// 405：服务区禁止使用该方法

// 500: 服务器在处理请求时候发生了错误
// 502: 服务起本射没有问题，访问的时候除了错误
// 503: 服务器处于停机维护或者负载，无法处理请求
...
// CONNECT: 简历资源链接隧道
// TRACE: 追踪请求对应的传输路径
// OPTIONS: 列出可对资源实行的请求fangaf

// 他允许浏览器向跨院服务器发送XMLHTTprequest请求，从而客服了ajax只能同源 使用

// cors的关键是服务起内部要实现CORS，才能完成

// JSONP请求

// 1 客户端准备一个接受数据的全局函数
// 2 客户端解析到外联script标签，发送请求 （只能发送GET请求）
// 3 服务器收到请求，返回函数的调用
// 4 客户端收到数据，执行回调


// 客户端准备一个能够接受数据的全局函数，
// 客户端解析外联的script标签，发送请求
// 服务器收到请求后，返回函数的调用
// 客户端收到数据，执行回调

// 客户端准备一个能够接受全局数据的函数
// 客户端解析外联script标签，发送请求
// 服务器收到请求后，返回函数的调用 
// 服务器收到请求后，返回函数的调用
// 客户端收到数据后，执行回调

// JSOP和AJAX请求的异同
// JSONP解决跨域的一种方案，通过客户端的script标签发出的请求方式

// 协议域名端口都要相同 同源策略
// 通过一些标签发出的请求不会被同源检查，比如script标签和img标签等等,JSONP就是通过script标签做的请求
// 相同点: 都是从客户端向服务器啊发送请求，讲数据拿回客户端进行处理

// 不同点 ajax是用xhr对象去实现，jsonp是用scirpt标签来实现的
// ajax是一个异步请求，JSONP是同步请求
// AJAX存在同源检查，jsonp不存在同源检查后端无需做解决跨域的方式
// ajax支持各种请求的执行方式，JSONP只支持GET请求
// ajax使用方式更加便捷，JSONP较为麻烦


// JSONP就是前端通过script标签不受同源策略的影响的特性，
// 向API接口拼接一个回调给服务端
// 然后服务端拿到这个回调， 执行并将需要的数据放入回调的参数里面
// 然后服务器返回数据给浏览器
// 所以这种跨域方式需要前端和后端配合
...
//     console.log(data);
// }
// `<script src="http:localhost:8080?callback=callback"></script>`

// JSONP的实现流程

// 生命一个回调函数，其函数名当做参数值，要传递给跨域请求数据的服务器，函数形参要为获取目标数据（）返回的data

// 创建一个script标签， 把那个跨域API接口的地址， 赋值给script在这个而地址的函数命
// 服务器收到请求后， 需要进行特殊处理， 把数据传递处去

// 最后服务器把准备id数据哦通过HTTP协议返回给客户端

// JSONP实现流程

// 1: 客户端先定义一个接受数据的全局函数
// 2: 动态生成script标签，发送请求，把回调函数当作参数传给服务器
// 3: 服务器接受到请i去，返回该函数的调用，并且把数据作为参数一起返回
// 4: 客户端执行回调，拿到数据

// 客户端先定义一个接受数据的全局函数
// 动态生成scirpt标签，发送请求，然后将函数作为参数传给服务器
// 服务器接受到请求，返该函数的调用，并且把数据作为参数一起返回
// 客户端执行回调，拿到数据

// 客户端先定义一个接受数据的全局函数
// 动态生成script标签，然后发送请求，将函数作为参数传给服务器
// 服务器接受到请求，返回该函数的调用，并把数据作为参数一起返回
// 客户端执行回调，拿到数据

// 客户端先定义一个接受数据的全局函数
// 动态生成script标签，发送请求，将函数作为参数传递给服务器
// 服务器受到请求，返回该函数的调用，并且把数据作为参数一起返回
// 客户端执行回调，拿到数据


// 客户端先定义一个接受数据的全局函数
// 动态生成script标签，将回调函数作为参数传入服务器
// 服务器接受到请i去，返回该函数的调用，并且把数据作为参数一并返回
// 客户端执行回调，拿到数据

// 客户端先定义一个接受数据的全局函数
// 动态生成script标签，将回调函数传入服务器
// 服务器接收到请求，返回该函数的调用，并且把数据作为参数一并返回
// 客户端执行回调，拿到数据
// 使用额外的HTTP头来告诉浏览器让运行在某一个origin上的Web应用能够允许访问来自不同源服务器上执行的资源

// CORS原理: 使用额外的HTTP头来告诉浏览器让运行在某一个origin上的web应用允许访问来自不同源服务器上的指定的资源

// 实现原理 // Access-Control-Allow-Origin

// 1. 如果是简单请求，会在请求头中增加一个Origin字段，表示这是来自哪一个源，而服务器接收到请求之后，回返回一个响应，响应头中
// 会包含一个Access-Control-Allow-Origin的字段，她的值要么包含由Origin首部字段所指明的域名，要么是"*"，表示接受任意域名的请求，如果响应头中没有这个字段，就说明当前这个源不在服务器许可范围内，报错
...
// 首先搭建一个代理服务器，接受客户端的请求，转发给服务器，拿到响应后，再将响应转发给客户端

// 跨域 不满足同源策略的两个源之间进行交互

// memory cache 和 disk cache

// 大的丢进磁盘 小的丢进内存

// 磁盘存储容量，存储时长 给disk cache

// 浏览器缓存：包括强缓存和协商缓存

// 首先检查强缓存， expires是过期时间没有超过这个过期时间的话可以直接从缓存中获取数据
// cache - control: max - age： 过期时长， 表示在这个时长内可以使用缓存， cache - control的优先级更高， 符合强缓存就返回200， 从本地缓存中拿取数据
// 当强缓存失效的时候，走协商缓存，浏览器携带缓存标识发送请求，协商缓存生效返回304，不生效，就返回200和全新的资源，比较的是last-modifyed(资源最后修改时间)和if-modifyed since 还有Etag(唯一标识)和 if-none-Math

// Vue双向数据绑定采用数据劫持结合发布订阅者模式，通过Object.defineProperty()设置各个属性的getter和setter，发布消息给订阅者，触发响应的监听回调

// 监听器：对数据进行遍历，包括子对象的属性，通过Object.defineProperty()设置各个属性的getter和setter，监听数据变化
// 解析器：解析Vue模板指令，将变量替换成数据，每个指令对应的节点绑定更新函数，添数据的订阅者，一旦数据改变，调用更新函数进行数据更新
// 订阅者：监听器和解析器通信的桥梁，主要是订阅监听器中属性的变化，当属性变化的时候，调用自身的update方法，触发解析器中的更新函数，更新视图
// 订阅器：采用发布订阅者模式，用来手机订阅者，对监听器和订阅者进行统一管理


// 执行同步任务，同步任务不用做特殊处理，直接执行
// 从宏任务队列中取出队头任务执行
// 如果产生了宏任务，将宏任务放入宏任务队列，下次轮询的时候执行
// 如果产生了微任务，将微任务放入微任务队列
// 执行完当前的宏任务后，取出微任务队列中的所有任务一次执行
// 如果韦尔努乌执行的过程中产生了新的微任务，则继续执行微任务，知道微任务队列为控
// 轮询

// 执行同步任务，同步任务不需要做任何处理直接执行
// 从宏任务队列中取出队头任务执行
// 如果产生了宏任务，就放入宏任务队列，下次轮询的时候执行
// 如果产生了微任务，就放入微任务队列
// 当前宏任务执行完毕，取出微任务队列中所有任务一次执行
// 如果微任务执行过程中产生了新的微任务，那就继续执行微任务，知道微任务队列为控

// JS代码执行时，主线程会从上到下一步步执行代码，同步任务依次放入到执行栈中主席那个，异步任务会在拿到结果的时候将注注册的回调函数放入任务队列，当当执行站中没有任务在执行的时候，引擎会在任务队列中读取任务压入执行栈中执处理执行
// 执行栈

// 跨站脚本攻击：浏览器中执行恶意代码，拿到用户的信息进行相关操作

// 反射型XSS：
// 存储行XSS：
// DOM型XSS:

// DNS域名解析：实际上是一个域名和IP对应的数据库
// DNS域名解析：域名和IP对应的数据库

// 第一次握手：客户端向服务器发送请求SYN,等待服务器回复
// 第二次握手，服务器收到请求并确认，回复一个SYN+ACK
// 第三次握手，客户端收到服务器回复指令并返回确认ACK

// 第一次握手：客户端发送SYN包给服务器，等待服务器确认

// BFC块级格式化上下文，是一个拥有独立渲染区域的盒子，盒子内部元素和外部元素不会互相影响

// 内部的盒子会在垂直方向上一个接一个的放置
// 计算BFC高度的时候，浮动的元素也参与计算
// BFC就是一个拥有独立渲染区域的盒子，盒子内部元素与外部元素互不影响
// 同一个BFC内的相邻的两个盒子边距重叠
...
// 输入过滤 输出转义 使用HTTPonly：JS脚本呢禁止访问Cookie
// 开启CSP，阻止白名单意外的资源加载或者与西宁


// CSRF 跨站请求伪造：用户登陆一个网站之后，产生了一个cookie，此时打开了一个新的网址，该网址返回了一些恶意的请求
// 不能获取到cookie只是使用cookie

// 利用Cookie的samesite属性
// strict：禁止第三方携带cookie
// lax：get方法和a标签可以携带cookie
// none：默认会自动携带上cookie
// 验证来源站点：两个请求字段 origin header和 referer header
// CSRF TOKEN  页面提交请求时候携带这个token进行验证不合法就不给响应

// JS代码执行的时候，主线程从上到下一步步执行代码，同步代码放依次放入执行栈中执行，异步任务会在拿到结的时候将回调函数放入任务队列，当执行栈中没有任务在执行的是hi和，引擎会在任务队列中读取任务压入执行栈中执行

// 浏览器中执行恶意脚本，然后拿到用户

// 执行同步任务，同步任务不需要任何处理直接执行
// 取出宏任务队头任务执行
// 如果产生了宏任务，放入宏任务队列，下次轮询的时候执行
// 如果产生了微任务，放入微任务队列
// 执行完当前的宏任务之后，取出微任务队列中的所有任务依次执行
// 如果微任务执行的过程中产生了新的微任务，则继续执行微任务，直到微任务队列为空
// 轮询意思昂

// vue双向数据绑定采用数据劫持结合发布订阅者模式，通过Object.definePorperty()对各个属性设置geter和seetter，发布i希奥希给订阅者，触发响应的监听回调

// 监听器：对数据对象进行遍历，包括子对象的属性，通过Object.definePropertY()对属性都加上setter和geteer，监听数据拜年话
// 解析器：解析Vue模板指令，将变量替换成数据，每个指令对应的节点绑定更i性能函数，添加监听数据的监听者，一旦数据改变的时候，调用更新函数更新数据
// 订阅者： 监听器和解析器沟通的桥梁，主要是订阅监听器中属性的变化，当属性变化的时候，调用自身的update方法，触发解析器中的更新哈桑农户，更细女尸图
// 订阅器： 订阅器采用发布-订阅模式，用来手机订阅者，对订阅者和监听器进行统一管理

// 闭包： 能够访问另一个函数作用域中的变量的函数

// 101: 在HTTP升级为WEBSOKET的时候，服务器统一就返回1001
// 200：客户端的请求在服务器得到正确处理
// 204：客户端请求陈工，但是没有资源返回
// 206：客户端进行范围请求，服务器执行了这部分的GET请求

// 301：永久重定向，资源背分配给了新的URL
// 302：临时重定向资源背临时分配给了新的URL
// 303：资源存在着另一个URL，需要用GET方法来获取
// 304：服务器告诉客户端我的资源没有改变，你直接用你本地的就可以
// 307：临时i重定向，资源背临时分配给了新的URL
...
// 502：服务器本身没有错误，访问的时候出了错误
// 503：服务器处于停机维护或者负载，无法粗合理请求
...
// 101: 在HTTP升级为websoket的时候，服务器统一变更，发送101

// 200: 客户端的请求被服务器正确处理
// 204: 请求成功，但是没有资源返回
// 207: 客户端进行了范围请求，服务器响应了这部分的GET请求

// 301：永久重定向，资源被永久分配给了新的URL
// 302：临时重定向，资源被临时分配给了新的URL
// 303: 标识资源存在着另一个URL，用GET方法来获取改资源
// 304: 服务器告诉客户端我的资源没有改变，你直接用你本地的就可以
// 307: 临时重定向，资源被临时分配给了新的URL
...
// 500: 服务器在处理请求的时候发生了错误
// 502: 服务七本身没有错误，访问的时候发生了错误
// 503: 服务器处于停机维护或者负载，无法处理请求
...
// JS代码执行的时候，主线程从上到下一步步执行代码，同步任务会被依次加入到执行栈中之心，异步任务会在拿到结果的是hi偶将回调函数放入任务队列，党执行站中没有任务的时候，引擎会在任务队列中读取任务压入执行栈中处理执行

// 1.执行同步任务，不需要做任何处理
// 2.从宏任务队列中取出队头的宏任务执行
// 3.如果产生了宏任务，将宏任务放入宏任务队列，下次轮询的时候执行
// 4.如果产生了微任务，将微任务放入微任务对垒
// 5.当前宏任务执行完毕，取出微任务队列中的所有任务依次执行
// 6.如果微任务执行的过程中产生了新的微任务，则继续执行微任务，直到微任务队列为控
// 轮询
// const args = [...arguments].slice(1)
// const args = Array.prototype.slice.call(arguments, 1)
// const args = Array.from(arguments.slice(1)) // 将aruments转换成数组

// vue双向数据绑定是数据劫持结合发布订阅者模式，通过Object.defineProperty()设置各个属性的setter和getter，数据更新的时候，发布消息给订阅者，触发响应的监听回调

// 监听器: 对数据对象进行遍历，包括子对象的属性，利用Object.defineProperty()设置各个属性的seter和getter，监听数据变化
// 解析器: 解析Vue模板指令，将变量替换成数据，每个指令对应的节点绑定更新函数，添加监听数据的订阅者，一旦数据改变。调用更新函数进行数据更新
// 订阅者：是监听器和解析器痛惜拿到桥梁，主要是订阅监听器中的属性变化，当属性变化的时候，触发自身的updat的方法，触发解析器中的更新函数，更新视图
// 订阅器：采用发布订阅者模式，手机订阅者，对订阅者和监听器进行管理


// 首先检查强缓存，expires是过期时间，没有超过这个过期时间的话直接从缓存中获取数据
// cache-control：max-age: 过期时长 标识在这个时间内可以使用缓存，优先使用cache-control 符合强缓存条件就返回200，从缓存中获取
// 当强缓存失效的时候，走协商缓存，浏览器携带缓存标识发送请求，协商缓存生效，返回304，不生效就返回新的资源和状态码200，比较的是last-modified(资源最后修改事件)和if modifyed since还有Etag(唯一标识)和 if none match是否一致
